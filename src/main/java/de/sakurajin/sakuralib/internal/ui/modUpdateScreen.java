    ...
    private boolean checkIfDiffAllowsJoin(){
        ...
        for(var entry:updateInfo.entrySet()){
            ...
            if(oldFormat > newFormat){
                return false;
            }else if(SakuraLib.CONFIG.WARN_ON_FORMAT_VERSION_UPDATE() && newFormat > oldFormat){
                return false;
            }
            ...
            try {
                ...
                if(majorComapre < 0){
                    return false;
                }else if(SakuraLib.CONFIG.WARN_ON_MAJOR_VERSION_UPDATE() && majorComapre > 0){
                    return false;
                }

                int allComapre = oldVersion.compareTo(newVersion);
                if(allComapre < 0){
                    return false;
                }else if(SakuraLib.CONFIG.WARN_ON_MINOR_VERSION_UPDATE() && allComapre > 0){
                    return false;
                }
